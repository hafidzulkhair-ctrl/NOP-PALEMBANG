...
st.markdown(
    ...
    <style>
        ...
        @media (max-width: 768px) {{
            ...
            /* Header judul jadi ditumpuk vertikal, logo & teks lebih kecil.
               Baris 1: logo Telkomsel + judul (sejajar, tidak wrap).
               Baris 2: logo NOP, rata tengah, ukuran lebih besar & jelas. */
            .app-header {{
...
st.markdown(
    f"""
    <div class="app-header" style="padding:20px 28px;border-radius:16px;
        background:linear-gradient(120deg,{TSEL_RED} 0%,{TSEL_ORANGE} 55%,{TSEL_YELLOW} 100%);
        margin-bottom:22px;box-shadow:0 6px 20px rgba(255,122,26,0.28);
        display:flex;align-items:center;justify-content:space-between;gap:20px;">
        <div style="display:flex;align-items:center;gap:20px;">
            <img src="data:image/png;base64,{LOGO_B64}" style="height:38px;" />
            <div>
                <h2 style="color:white;margin:0;letter-spacing:0.5px;text-shadow:0 1px 4px rgba(0,0,0,0.35);">AVA ANALYSIS DASHBOARD</h2>
                <p style="color:#FFF3DC;margin:2px 0 0 0;text-shadow:0 1px 3px rgba(0,0,0,0.30);">TSEL NOP Palembang</p>
            </div>
        </div>
        <img class="app-header-nop-logo" src="data:image/png;base64,{NOP_LOGO_B64}"
            style="height:56px;flex-shrink:0;filter:drop-shadow(0 2px 6px rgba(0,0,0,0.35));" />
    </div>
    """,
    unsafe_allow_html=True,
)
...
c3, c4 = st.columns(2)
c3.metric("Rentang Week", f"{ALL_WEEKS[0]} - {ALL_WEEKS[-1]}")
    ...
    with colg2:
        if len(grafik_weeks_all) >= 2:
            week_range = st.select_slider(
                ...
                value=(grafik_weeks_all[0], grafik_weeks_all[-1]),
                key="grafik_range",
            )
        else:
